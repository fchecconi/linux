...
{
	struct sched_param2 param2 = { .sched_priority = param->sched_priority };
		       ...
		       struct rt_bandwidth *rt_b)
{
	...
	hrtimer_init(&rt_rq->rt_period_timer, CLOCK_MONOTONIC,
		     HRTIMER_MODE_REL);
	rt_rq->rt_period_timer.irqsafe = 1;
...
#ifdef CONFIG_RT_GROUP_SCHED
static void init_tg_rt_entry(struct task_group *tg,
			     struct rt_rq *rt_rq, int cpu)
{
	struct rq *rq = cpu_rq(cpu);
				    ...
				    struct rt_schedulable_data *d)
{
	...
	list_for_each_entry_rcu(child, &tg->children, siblings) {
		if (d && child == d->tg && !d->rt_task_data) {
			...
			rt_tg_parameters(child, false, &period, &runtime);
...
static ssize_t cpu_rt_multicpu_runtime_write_string(struct cgroup *cgrp,
						    struct cftype *cft,
						    const char *buf)
{
	struct task_group *tg = cgroup_tg(cgrp);
