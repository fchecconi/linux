 ...
 */
#define RT_MIN_RUNTIME_NSEC	((u64)500)

static inline bool move_runtime(struct rt_rq *rt_rq, struct rt_rq *iter,
				u64 target, int weight, u64 rt_period)
{
	...
	if (iter->rt_nr_running)
		...
		leave = max(RT_MIN_RUNTIME_NSEC, iter->rt_time);
...
static void do_balance_runtime(struct rt_rq *rt_rq)
{
	...
	int i, weight;
	u64 rt_period, prev_runtime, target;
	bool stop = false;
	...
	if (rt_rq->rt_runtime < rt_b->rt_runtime) {
		...
		/* Our contribution to weight should make it at least one. */
		BUG_ON(!weight);

		/* Aim at our share plus 1/weight of all the spare bw. */
		target = rt_b->rt_runtime + (cpumask_weight(rd->span)-weight)
			  * (rt_b->rt_runtime - RT_MIN_RUNTIME_NSEC) / weight;
	}
	...
	for_each_cpu(i, rd->span) {
		...
		stop = move_runtime(rt_rq, iter, target, weight, rt_period);
...
static int sched_rt_runtime_exceeded(struct rt_rq *rt_rq)
{
	...
	if (rt_rq->rt_time >= rt_rq->rt_runtime) {
		...
		if (likely(rt_b->rt_runtime)) {
			 ...
			 */
			rt_rq->rt_time = 0;
			printk_once(KERN_WARNING "sched: RT throttling bypassed\n");
		}
